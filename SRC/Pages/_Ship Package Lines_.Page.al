page 55004 "Ship Package Lines"
{
    ...
    layout
    {
        area(content)
        {
            repeater(General)
            {
                ...
                field("Box/Pallet Weight";Rec."Box/Pallet Weight")
                {
                    ApplicationArea = all;
                    Editable = false;
                }
                ...
                field("New Packing No.";Rec."New Packing No.")
                {
                    ...
                    trigger OnLookup(var Text: Text): Boolean var PackingList: Record "Packing List";
                    begin
                        PackingList.Reset();
    ...
    I: Integer)var JsonObjectVar: JsonObject;
    ShipPackageHeader: Record "Ship Package Header";
    ...
    local procedure SetShippingToAddress(ShipPackageHeader: Record "Ship Package Header")var myInt: Integer;
    JsonObjectVar: JsonObject;
    begin
        Clear(JsonObjectVar);
    ...
    local procedure SetShippingFromAddress(Location: Record Location)var JsonObjectVar: JsonObject;
    PackingModuleSetup: Record "Packing Module Setup";
    begin
        PackingModuleSetup.Get();
        Clear(JsonObjectVar);
        ...
        if PackingModuleSetup."Mode Test" = true then JsonObjectVar.Add('mode', 'test')
        else
    ...
    local procedure ParcelInfo(palletBoxMaster: Record "Pallet/Box Master")var JsonObjectVar: JsonObject;
    begin
        Clear(JsonObjectVar);
    ...
    local procedure Custom_info(ShipPackageLines: Record "Ship Packing Lines")var JsonObjectVar: JsonObject;
    shipPackageheader: Record "Ship Package Header";
    begin
        Clear(JsonObjectVar);
    ...
    ShipPackageHeader: Record "Ship Package Header")var begin
        ShipPackageHeader.Reset();
        ShipPackageHeader.SetRange(No, Rec."No.");
        if ShipPackageHeader.FindFirst()then;
    ...
    var "Pallet/Box master": Record "Pallet/Box Master";
    Location: Record Location;
    ShipPackageHeader: Record "Ship Package Header";
